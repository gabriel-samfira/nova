    ...
    def _soft_shutdown(self, instance,
        ...
        try:
            ...
            if self._wait_for_power_off(instance.name, timeout):
                LOG.info(_("Soft shutdown succeded."), instance=instance)
                return True
        except vmutils.HyperVException as e:
            # Exception is raised when trying to shutdown the instance
            # while it is still booting.
            LOG.warning(_("Soft shutdown failed: %s"), e, instance=instance)
            return False

        LOG.warning(_("Timed out while waiting for soft shutdown."),
                    instance=instance)
        return False
